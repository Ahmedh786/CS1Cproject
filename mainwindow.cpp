...
{
    ofstream Outfile;
    Outfile.open("/home/cs1c/fall2018/gitstuff5/CS1Cproject/CS1Cofficial/shapes.txt");
    for (int i = 0; i < 5/*data::shapeVector.getSize()*/; ++i) {
        Outfile << "-Shape Id: " << data::shapeVector[i]->getShapeId(); << endl;
        Outfile << "ShapeType: " << data::shapeVector[i]->getTextString(); << endl;
        if (data::shapeVector[i]->getTextString() == "Line")
            ...
            Outfile << "ShapeDimensions: " << data::shapeVector[i]->getX() << ", " << data::shapeVector[i]->getY() << ", " << data::shapeVector[i]->getL() << ", " << data::shapeVector[i]->getW() << endl;
        Outfile << "PenColor: " << data::shapeVector[i]->getQPenColor() << endl;
        Outfile << "PenWidth: " << data::shapeVector[i]->getQPenWidth() << endl;
        Outfile << "PenStyle: " << data::shapeVector[i]->getQPenStyle() << endl;
        Outfile << "PenCapStyle: " << data::shapeVector[i]->getQPenCapStyle() << endl;
        Outfile << "PenJoinStyle: " << data::shapeVector[i]->getQPenJoinStyle() << endl;
        Outfile << endl;
    }
    Outfile.close();
